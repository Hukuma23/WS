...
void SwIn::init(byte scl, byte sda) {
	DEBUG4_PRINTF2("SwIn.init scl=%d, sda=%d", scl, sda); DEBUG4_PRINTLN();
	Wire.pins(scl, sda);
	Wire.begin();
...
void SwIn::longtimeHandler() {
	...
	if (act_state == LOW) {
		...
		if (mqtt != NULL)
			mqtt->publish(AppSettings.topMIN, AppSettings.getMINbyPin(pin), OUT, String("LONG"));
	}
	else {
		DEBUG4_PRINTLN("*-pressed-*");
		if (mqtt != NULL)
			mqtt->publish(AppSettings.topMIN, AppSettings.getMINbyPin(pin), OUT, String("PRSD"));
	}
