...
byte dsAddr[2][8] = {
  {0x28, 0xFF, 0x23, 0x15, 0x71, 0x14, 0x04, 0xB5},
  {0x28, 0xFF, 0xB3, 0x15, 0x71, 0x14, 0x04, 0x02}
};
/*
 * {0x28, 0xFF, 0xB1, 0x19, 0x71, 0x14, 0x04, 0x93},
 * {0x28, 0xFF, 0x53, 0xDA, 0x70, 0x14, 0x04, 0xD0},
 */
...
/*
void setSwOutput(uint8_t sw) {
  
  DEBUG_PRINT("sw=");
  DEBUG_PRINTLN(sw);

  DEBUG_PRINT("before set, sw0, 1, 2, 3, 4 =");
  DEBUG_PRINT(swState[0]);
  DEBUG_PRINT(", ");
  DEBUG_PRINT(swState[1]);
  DEBUG_PRINT(", ");
  DEBUG_PRINT(swState[2]);
  DEBUG_PRINT(", ");
  DEBUG_PRINT(swState[3]);
  DEBUG_PRINT(", ");
  DEBUG_PRINT(swState[4]);
  DEBUG_PRINTLN(); 
  ...
  if (state != swState[0]) {
    swState[0] = state;
    digitalWrite((swPin[0]), state);
  }
  
  state = (((sw & 2)  >> 1) == 1)?HIGH:LOW;
  if (state != swState[1]) {
    swState[1] = state;
    digitalWrite((swPin[1]), state);
  }
  
  state = (((sw & 4)  >> 2) == 1)?HIGH:LOW;
  if (state != swState[2]) {
    swState[2] = state;
    digitalWrite((swPin[2]), state);
  }
  
  state = (((sw & 8)  >> 3) == 1)?HIGH:LOW;
  if (state != swState[3]) {
    swState[3] = state;
    digitalWrite((swPin[3]), state);
  }
  state = (((sw & 16) >> 4) == 1)?HIGH:LOW;
  if (state != swState[4]) {
    swState[4] = state;
    digitalWrite((swPin[4]), state);
  }
  DEBUG_PRINT("after set sw0, 1, 2, 3, 4 =");
  DEBUG_PRINT(swState[0]);
  DEBUG_PRINT(", ");
...
uint8_t genSw (void) {
  ...
  DEBUG_PRINTLN(); 
  uint8_t sw;
  sw = (getState(0)) + (getState(1) << 1) + (getState(2) << 2) + (getState(3) << 3) + (getState(4) << 4);
