...
private:
	...
	void init(IPAddress broker_ip, int broker_port, unsigned int shift = DEFAULT_SHIFT, unsigned int interval = DEFAULT_INTERVAL);
	byte connect();
	void disconnect();
...
public:
	MQTT(IPAddress broker_ip, int broker_port);
	MQTT(IPAddress broker_ip, int broker_port, unsigned int shift, unsigned int interval);
	MQTT(IPAddress broker_ip, int broker_port, unsigned int shift, unsigned int interval, String topicMain, String topicClient);
	virtual ~MQTT();
