...
void var_init() {
	...
	topDSTemp = "DS_Temperature";
	topBMPTemp = "BMP_Temperature";
	topBMPPress = "BMP_Pressure";
...
void onMessageReceived(String topic, String message) {
	...
	for (byte i = 0; i < AppSettings.sw_cnt; i++) {
		...
		if (topic.equals(objMQTT.getTopic(topSw, (i+1), IN))) {
			if (message.equals("ON")) {
				turnSw(i, HIGH);
			} else if (message.equals("OFF")) {
				turnSw(i, LOW);
			} else
				DEBUG4_PRINTF("Topic %s, message is UNKNOWN", (objMQTT.getTopic(topSw, (i+1), IN)).c_str());
		}
	}
	...
	for (byte i = 0; i < AppSettings.ssw_cnt; i++) {
		if (topic.equals(objMQTT.getTopic(sTopSw, (i+1), IN))) {
			if (message.equals("ON")) {
				turnSsw(i, HIGH);
			} else if (message.equals("OFF")) {
				turnSsw(i, LOW);
			} else
				DEBUG4_PRINTF("Topic %s, message is UNKNOWN", (objMQTT.getTopic(sTopSw, (i+1), IN)).c_str());
		}
	}
	// *** Serial block end ***
	if (topic.equals(objMQTT.getTopic(topCfg, IN))) {
		...
		if (cmd.equals("sw_update")) {
			...
			objMQTT.publish(topCfg, OUT, "Will stop all timers and UPDATE on the air firmware only now");
			OtaUpdateSW();
		}
			...
			objMQTT.publish(topCfg, OUT, "Will stop all timers and UPDATE on the air firmware and spiffs now");
			stopAllTimers();
			OtaUpdateAll();
		}
		else if ((cmd.equals("version")) || (cmd.equals("ver"))) {
			//mqtt.publish(topCfg_Out, AppSettings.version);
			objMQTT.publish(topCfg, OUT, AppSettings.version);
		}
		else if (cmd.equals("restart")) {
			//mqtt.publish(topCfg_Out, "Will restart now");
			objMQTT.publish(topCfg, OUT, "Will restart now");
			System.restart();
		}
		else if (cmd.equals("conf_del")) {
			//mqtt.publish(topCfg_Out, "Delete config now");
			objMQTT.publish(topCfg, OUT, "Will delete config now");
			...
			objMQTT.publish(topCfg, OUT, updList);

		}
		else if (cmd.equals("conf_save")) {
			AppSettings.save();
			//mqtt.publish(topCfg_Out, "Settings saved.");
			objMQTT.publish(topCfg, OUT, "Settings saved.");
		}
			...
			objMQTT.publish(topCfg, OUT, strPrint);
		}
			...
			objMQTT.publish(topCfg, OUT, strPrintf);
		}
		/*
...
void publishSerialSw() {
	...
	for (byte i = 0; i < ActStates.ssw_cnt; i++) {
		if (ActStates.ssw[i])
			objMQTT.publish(sTopSw, (i+1), OUT, "ON");	//mqtt.publish(sTopSw_Out+String(i+1), "ON");

		else
			objMQTT.publish(sTopSw, (i+1), OUT, "OFF");	//mqtt.publish(sTopSw_Out+String(i+1), "OFF");
	}
...
void publishSerial() {
	...
	// Publish DHT
	if (sDHTTemp != UNDEF) {
		result = objMQTT.publish("sdht_t", OUT, String(sDHTTemp));	//result = mqtt.publish(getOutTopic("sdht_t"), String(sDHTTemp));
		if (result)
			sDHTTemp = UNDEF;
	}

	if (sDHTHum != UNDEF) {
		result = objMQTT.publish("sdht_h", OUT, String(sDHTHum));	//result = mqtt.publish(getOutTopic("sdht_h"), String(sDHTHum));
		if (result)
			sDHTHum = UNDEF;
	}
	...
	// Publish BMP
	if (sBMPTemp != UNDEF) {
		result = objMQTT.publish("sbmp_t", OUT, String(sBMPTemp));	//result = mqtt.publish(getOutTopic("sbmp_t"), String(sBMPTemp));
		if (result)
			sBMPTemp = UNDEF;
	}
	if (sBMPPress != UNDEF) {
		result = objMQTT.publish("sbmp_p", OUT, String(sBMPPress));	//result = mqtt.publish(getOutTopic("sbmp_p"), String(sBMPPress));
		if (result)
			sBMPPress = UNDEF;
	}
	...
	for (int i=0; i < dsSize; i++) {
		if (sDSTemp[i] != UNDEF) {
			result = objMQTT.publish("sds_t", i, OUT, String(sDSTemp[i]));	//result = mqtt.publish(getOutTopic("sds_t") + String(i), String(sDSTemp[i]));
			if (result)
				sDSTemp[i] = UNDEF;
		}
	}
	...
	if ((sWaterCold > 0) && (sWaterCold != UNDEF)) {
		result = objMQTT.publish("s_wc", OUT, String(sWaterCold));	//result = mqtt.publish(getOutTopic("s_wc"), String(sWaterCold));
		if (result)
			sWaterCold = UNDEF;
	}
	if ((sWaterHot > 0) && (sWaterHot != UNDEF)) {
		result = objMQTT.publish("s_wh", OUT, String(sWaterHot));	//result = mqtt.publish(getOutTopic("s_wh"), String(sWaterHot));
		if (result)
			sWaterHot = UNDEF;
	}
...
void mqtt_loop() {
	...
	if (dsTemp[0] > 0) {
		for (int i = 1; i <= dsTemp[0]; i++) {
			...
			if (dsTemp[i] > -255) {
				...
				bool result = objMQTT.publish(topDSTemp, i, OUT, String(dsTemp[i]));	//bool result = mqtt.publish(topDSTemp_Out + String(i), String(dsTemp[i]));
	...
	if (dhtTemp > -255) {
		bool result = objMQTT.publish(topDHTTemp, OUT, String(dhtTemp));	//bool result = mqtt.publish(topTemp1_Out, String(dhtTemp));
		DEBUG4_PRINT(" ");
	...
	if (dhtHum > -255) {
		bool result = objMQTT.publish(topDHTHum, OUT, String(dhtHum));	//bool result = mqtt.publish(topHum1_Out, String(dhtHum));
		DEBUG4_PRINT(" ");
		DEBUG4_PRINT(dhtHum);
		DEBUG4_PRINT(" %... ");
		DEBUG4_PRINT("Published: ");
	...
	if (bmpTemp > -255) {
		bool result = objMQTT.publish(topBMPTemp, OUT, String(bmpTemp));	//bool result = mqtt.publish(topBMPTemp_Out, String(bmpTemp));
		DEBUG4_PRINT(" ");
	...
	if (bmpPress > -255) {
		...
		bool result = objMQTT.publish(topBMPPress, OUT, String(bmpPress));	//bool result = mqtt.publish(topBMPPress_Out, String(bmpPress));
		DEBUG4_PRINT(" ");
...
void serialCollector() {
	...
	objMQTT.publish(topLog, OUT, "serialCollector() cmd = " + String(SerialCommand::COLLECT) + " objType=" + String(ObjectType::ALL) + " objId=" + String(ObjectId::ALL));
}
...
void serialReceiver() {
	...
	objMQTT.publish(topLog, OUT, "serialReceiver() cmd = " + String(SerialCommand::GET) + " objType=" + String(ObjectType::ALL) + " objId=" + String(ObjectId::ALL));
}
...
void processSerialMessage() {
	...
	objMQTT.publish(topLog, OUT, "processSerialMessage() cmd = " + String(cmd) + " objType=" + String(objType) + " objId=" + String(objId));
	/*
    DEBUG4_PRINTLN();
    DEBUG4_PRINT("cmd=");
    DEBUG4_PRINT(cmd);
...
void startTimers() {
	...
	objMQTT.setTopic(AppSettings.main_topic, AppSettings.client_topic);
	objMQTT.init(AppSettings.broker_ip, AppSettings.broker_port, AppSettings.shift_mqtt, AppSettings.interval_mqtt, onMessageReceived);

	objMQTT.startTimer(mqtt_loop);
	delay(50);
	...
	//DEBUG4_PRINTLN(objMQTT.getState());
	...
	DEBUG4_PRINTLN("startTimers. DONE!");
}
...
void publishSwitches() {
	...
	for (byte i = 0; i < ActStates.sw_cnt; i++) {
		if (ActStates.getSw(i))
			objMQTT.publish(topSw, (i+1), OUT, "ON");	//mqtt.publish(topSw_Out + String(i+1), "ON");
		else
			objMQTT.publish(topSw, (i+1), OUT, "OFF");	//mqtt.publish(topSw_Out + String(i+1), "OFF");
	}
